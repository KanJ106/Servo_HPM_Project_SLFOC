 ...
 */
extern CO_ReturnError_t TPDO_SendFast(CO_CANmodule_t *CANmodule, CO_CANtx_t *txbuf);
...
static CO_ReturnError_t CO_TPDOsend(CO_TPDO_t *TPDO)
{
    ...
    for (uint8_t i = 0; i < PDO->mappedObjectsCount; i++)
    {
        ...
        /* Set stream.dataOffset to zero, perform OD_IO.read() and store
         * mappedLength back to stream.dataOffset */
        stream->dataOffset = 0;
        OD_size_t countRd;
    ...
    return TPDO_SendFast(PDO->CANdev, TPDO->CANtxBuff);
}
