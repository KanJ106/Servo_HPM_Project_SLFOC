        ...
        if (OD_1006_period > 0U) {
            ...
            if (SYNC->isProducer) {
                ...
                if (SYNC->timeoutError == 1U) {
                    /* P7-18 specifies how many consecutive SYNC periods may be missed. */
                    uint32_t syncLostLimit = 2U;
                    if ((RamCommu != NULL) && (RamCommu->SMLost != 0U)) {
                        syncLostLimit = RamCommu->SMLost;
                    }

                    /* Calculate OD 0x1006 period * P7-18, saturating on overflow. */
                    uint32_t periodTimeout;
                    if (OD_1006_period > (0xFFFFFFFFU / syncLostLimit)) {
                        periodTimeout = 0xFFFFFFFFU;
                    } else {
                        periodTimeout = OD_1006_period * syncLostLimit;
                    }
