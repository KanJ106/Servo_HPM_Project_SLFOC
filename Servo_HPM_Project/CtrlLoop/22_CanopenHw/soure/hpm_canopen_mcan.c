...
uint8_t can_recvMsg_process(uint8_t sel)
{
    ...
    {   
        uint8_t fifolen = (can->RXF1S)&0x3F;
        for(uint8_t i = 0 ; i < fifolen ; i++)
        {
            hpm_mcan_get_message_from_rxfifo(&hpm_canopen_dev[0], 1);
...
int mcan_add_rx_filter(const struct device *dev,        //CAN Dev
                       can_rx_callback_t callback,      //接收数据后的回调函数
                       void *user_data,                 //回调函数的参数，当前为CANModule
                       const struct can_filter *filter, //过滤器
                       bool is_ext)                     //1为扩展帧
{
    hpm_can_config_t *cfg = dev->config;
    hpm_can_data_t *data = dev->data;
    ...
    if (is_ext) 
    {
        if (data->ext_filter_count < HPM_CAN_EXT_FILTER_NUM_MAX) 
        {
            ...
            //在创建扩展过滤器时，先检查是否有可用的过滤器，如果有则返回可用的过滤器ID
            for(uint32_t i = 0; i < 4; i++)
            {
                uint32_t filter_id_used_temp = data->filter_id_used[i];
                for(uint32_t j = 0; j < 32; j++)
                {
                    if((filter_id_used_temp & (1 << j)) == 0)
                    {
                        filter_id = i * 32 + j;
                        data->filter_id_used[i] |= (1 << j);
                        ret = 0;
                        break;
                    }
                }     
            ...
            filter_tmp = &data->ext_filters[data->std_filter_count - 1].filters;
            data->ext_filters[data->std_filter_count - 1].filters_index = filter_id;
    ...
    else 
    {
        if (data->std_filter_count < HPM_CAN_STD_FILTER_NUM_MAX) 
        {
            ...
            filter_tmp = &data->std_filters[data->std_filter_count - 1].filters;
            data->std_filters[data->std_filter_count - 1].filters_index = filter_id;
    ...
    //取 SYNC ID
    sync_id = CumObj.x1005_COB_ID_SYNCMessage;

    //分配 FIFO
    if((filter->id & sync_id) == sync_id)
    {
        filter_tmp->filter_config = MCAN_FILTER_ELEM_CFG_SET_PRIORITY_AND_STORE_IN_FIFO0_IF_MATCH;
    }
    else if( ( (filter->id & 0x780) == 0x200) || ((filter->id & 0x780) == 0x300) || ((filter->id & 0x780) == 0x400) || ((filter->id & 0x780) == 0x500))
    {
        filter_tmp->filter_config = MCAN_FILTER_ELEM_CFG_STORE_IN_RX_FIFO0_IF_MATCH;// MCAN_FILTER_ELEM_CFG_STORE_IN_RX_FIFO0_IF_MATCH
    }
    else
    {
        filter_tmp->filter_config = MCAN_FILTER_ELEM_CFG_STORE_IN_RX_FIFO1_IF_MATCH;// MCAN_FILTER_ELEM_CFG_SET_PRIORITY_AND_STORE_IN_FIFO1_IF_MATCH
    }
    
    filter_tmp->filter_id = filter->id;
    //filter_tmp->filter_mask = ~filter->mask; 
    filter_tmp->filter_mask = filter->mask; //MCAN中掩码是1有效，进行匹配
    ...
    status = mcan_set_filter_element(can, filter_tmp, data->std_filter_count - 1);
...
int hpm_mcan_add_rx_filter(const struct device *dev,         //CAN Dev   
                            can_rx_callback_t callback,      //接收数据后的回调函数
                            void *user_data,                 //回调函数的参数列表
                            const struct can_filter *filter) //过滤器   
{
    int filter_id;
...
bool mcan_remove_filter(MCAN_Type *ptr,       
                        hpm_can_data_t *data, 
                        int filter_id, 
                        bool is_ext)
{
    ...
    uint16_t write_filter_buf_index = 0;
    if (is_ext)
    {
        ...
        for (uint32_t i = 0; i < data->ext_filter_count; i++)
        {
            if (data->ext_filters[i].filters_index == filter_id)
            {
                ...
                //将对应的 filter_id 标记为已使用
                uint32_t filter_id_used_temp = data->filter_id_used[filter_id / 32];
                filter_id_used_temp &=  ~(1 << (filter_id % 32));
                data->filter_id_used[filter_id / 32] = filter_id_used_temp;

                write_filter_buf_index = i;
                break;
            }
        }
    }
    else
    {
        ...
        for (uint32_t i = 0; i < data->std_filter_count; i++)
        {
            if (data->std_filters[i].filters_index == filter_id)
            {
                ...
                {
                    is_last_index = false;
                    data->std_filters[i].filters_index = last_filter_index; // last_filter_index
                ...
                {
                    is_last_index = true;
                    data->std_filters[i].filters_index = 0;
                }

                //将对应的 filter_id 标记为未使用
                uint32_t filter_id_used_temp = data->filter_id_used[filter_id / 32];
                filter_id_used_temp &=  ~(1 << (filter_id % 32));
                data->filter_id_used[filter_id / 32] = filter_id_used_temp;
    ...
    if (find_index)
    {
        ...
        if (is_last_index)
        {
            ...
            current_filter_elem->can_id_type = 0;
            current_filter_elem->sync_message = 0;
            current_filter_elem->id2 = 0;
            current_filter_elem->id1 = 0;

            status = mcan_set_filter_element(ptr, current_filter_elem, write_filter_buf_index);
            if (status != status_success)
            {
        ...
        else
        {
            ...
            status = mcan_set_filter_element(ptr, current_filter_elem, data->std_filter_count - 1);
            if (status != status_success)
            {
                // printf("remove rx filter failed, error code: %d\n", status);
                return false;
...
CO_ReturnError_t TPDO_SendFast(CO_CANmodule_t *CANmodule, CO_CANtx_t *txbuf) 
{
    struct device *dev = CANmodule->CANptr;
    hpm_can_config_t *cfg = dev->config;
    MCAN_Type *can = cfg->base;
    hpm_stat_t status;
    ...
    // 2. 检查 TX FIFO 是否有可用空间（精简：不检查则可能丢失帧）

    // 3. 直接构建 MCAN 发送帧
    mcan_tx_frame_t tx_frame;
    memset(&tx_frame, 0, sizeof(tx_frame));
    tx_frame.std_id = txbuf->ident;
    tx_frame.dlc = txbuf->DLC;     // 数据长度 
    tx_frame.bitrate_switch = 0;   // 固定帧率
    tx_frame.rtr =  0;             // 非远程帧
    tx_frame.canfd_frame = 0;      // 不支持 CAN FD

    // 拷贝数据
    for (int i = 0; i < txbuf->DLC; i++)
    {
        tx_frame.data_8[i] = txbuf->data[i];
    }

    // 4. 写入 FIFO（非阻塞）
    uint32_t fifo_index = 0;
    status = mcan_transmit_via_txfifo_nonblocking(can, &tx_frame, &fifo_index);
                  ...
                  const struct can_frame *frame)
{
    ...
    uint32_t fifo_index = 0;
    status = mcan_transmit_via_txfifo_nonblocking(can, &tx_buf, &fifo_index);
...
void hpm_mcan_init(struct canopen_context *candriverstate, can_info_t *can_info, uint32_t baudrate, int index)
{
    ...
    //FIFO覆盖模式 FIFO消息满了丢弃最开始的数据
    can_config.ram_config.rxfifos[0].operation_mode = 1;
    can_config.ram_config.rxfifos[1].operation_mode = 1;
    ...
    //                            MCAN_EVENT_ERROR;
    can_config.interrupt_mask = MCAN_INT_HIGH_PRIORITY_MSG | MCAN_EVENT_ERROR;
    ...
    hpm_canopen_data[index].filter_id_used[0] = 0; //过滤器ID使用情况
    hpm_canopen_data[index].filter_id_used[1] = 0; 
    hpm_canopen_data[index].filter_id_used[2] = 0; 
    hpm_canopen_data[index].filter_id_used[3] = 0; 
